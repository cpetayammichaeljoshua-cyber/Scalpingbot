# ═══════════════════════════════════════════════════════════════════════════════
# Unity Engine v18.93 — Multi-stage Production Dockerfile
...
# requirements.txt v18.93: torch/transformers EXCLUDED — installed above separately.
# --extra-index-url included as fallback only (no conflict since torch absent here).
COPY requirements.txt .
RUN pip install --prefix=/install --no-cache-dir --root-user-action=ignore \
    --timeout 300 --retries 3 \
    -r requirements.txt \
    && echo "OK requirements.txt installed (v18.93)"
