# ═══════════════════════════════════════════════════════════════════════════════
# Unity Engine v18.90 — Multi-stage Production Dockerfile
# Optimised for Railway.app deployment
#
# Build:  docker build -t unity-engine:18.90 .
...
#   • Version labels updated to 18.90.
...
# requirements.txt v18.90: torch/transformers EXCLUDED — installed above separately.
# --extra-index-url included as fallback only (no conflict since torch absent here).
COPY requirements.txt .
RUN pip install --prefix=/install --no-cache-dir --root-user-action=ignore \
    --timeout 300 --retries 3 \
    -r requirements.txt \
    && echo "OK requirements.txt installed (v18.90)"
...
print('OK Unity Engine v18.90 — SOVEREIGN [1.00] dependency singularity verified')
"
...
LABEL maintainer="Unity Engine Bot" \
      version="18.90"               \
      description="Unity Engine v18.90 — 28-layer SOVEREIGN | 4-tier torch CDN | HMM/VPIN/Kalman/Dispersion/PCA/CSM/IVCrush | MVO/BL/Kelly/PBO | OpenRouter 38+ models | ZERO DEGRADED 0.75"
