...
def _bootstrap_all_critical_packages() -> None:
    ...
    if _can_import("torch") and not _can_import("transformers"):
        _log.info("⚡ [v18.62 Bootstrap] Installing transformers==5.8.0...")
        ...
        try:
            ...
            if _ttr.returncode == 0:
                try:
                    ...
                    pass
                _log.info("✅ [v18.62 Bootstrap] transformers==5.8.0 installed")
            else:
                _log.info("ℹ️  [v18.62 Bootstrap] transformers not available — torch.nn.TransformerEncoder is primary (no HF dependency)")
        except Exception as _te:
            _log.info(f"ℹ️  [v18.62 Bootstrap] transformers install skipped: {_te}")
...
AUTO_PAPER_WR_THRESHOLD = 0.38   # v18.85: 0.18→0.38 — CRITICAL FIX: at WR=30.2% the 0.18 threshold allowed live trading despite demonstrably negative EV (EV=-0.314R, Sharpe=-4.87); 0.38 enforces paper/shadow mode until rolling-20 WR ≥ 38% (minimum for positive EV at RR=2.20: need WR>0.70/2.20=31.8%; 38% adds +6pp safety margin); engine computes all signals in shadow mode but does not dispatch — capital preserved until genuine edge demonstrated; recovers automatically when WR improves
try:
...
EV_MIN_THRESHOLD      = 0.0028   # ≥+28bps EV after slippage required to accept a signal (v18.85: 14→28bps — CORE FIX: progressive lowering from 35→14bps directly caused EV=-0.3140R and WR=30.2%; 14bps EV gate was passing signals with near-zero expectancy that are noise at WR<35%; 28bps = 18bps headroom above 10bps round-trip slippage — matches original validated threshold from v15.5; at WR=30% need EV>14bps just to cover slippage + 14bps alpha margin = 28bps total institutional minimum; G0 pass-rate target ~45% to filter out bottom half by EV)
...
DEAD_ZONE_QUALITY_PENALTY = float(os.getenv("DEAD_ZONE_QUALITY_PENALTY", "5.0") or 5.0)  # quality penalty during dead-zone hours (v18.85: 3.0→5.0 — G0.5 was #1 bottleneck at 58% pass-rate; at raised quality floor of 62 a 5pt penalty requires base quality≥67 during dead zone 00-02h UTC; WR=22-23% in dead zone makes this correct; SOVEREIGN Markov (+16pts) still absorbs 5pt penalty cleanly; aligns dead-zone filter with new 62 institutional quality baseline)
...
SESSION_QUALITY_BONUS   = 6.0    # quality bonus during prime session overlap [v18.78: 4.0→5.0; v18.81: 5.0→5.5; v18.84: 5.5→6.0 — G0.5 #1 bottleneck (58%); each +0.5pt increment lifts borderline signals over IRONS floor; 6.0pts = prime window gives a full grade-level quality boost; Sortino-crisis threshold -4.0 from v18.81 means genuine crisis (Sortino<-4.0) gets 0.65×6.0=3.9pts (vs 3.6pts before)]
...
IRONS_MIN_WR_BELOW30  = 65.0    # WR < 30%  → elevated floor (v18.85: 57→65 — CORE FIX: at WR=30.2% we are IN this tier; 57 was only 1pt above SIGNAL_MIN_QUALITY_GATE=56, making Gate 10 completely redundant; 65 restores institutional separation: G9 floor=62, G10 floor=65 → only signals scoring 65+ on IRONS composite pass during sub-30% WR regime; at WR=30.2% this is the correct institutional IRONS floor to enforce signal conviction)
IRONS_MIN_WR_30_45    = 62.0    # WR 30-45% → moderately elevated (v18.85: 57→62 — IRONS floor aligned with SIGNAL_MIN_QUALITY_GATE=62; at 30-45% WR range a 57 floor was 1pt above the quality gate making it decorative; 62 creates co-equal G9/G10 dual floor; signals must score ≥62 on BOTH composite quality AND IRONS to pass; Markov-SOVEREIGN (+16pts) elevates to 78+ clearing both floors unanimously)
...
SOVEREIGN_RECOVERY_GATE   = float(os.getenv("UNITY_SOVEREIGN_RECOVERY_GATE", "65.0") or 65.0)  # v18.85: 59→65 — recalibrated to maintain +3pt buffer above raised SIGNAL_MIN_QUALITY_GATE=62 (was +3pt above old floor=56); SOVEREIGN_RECOVERY path now requires IRONS≥65 to pass, matching the new IRONS_MIN_WR_BELOW30=65 institutional floor; coherent two-tier filter: G9=62 + G10/SOVEREIGN_RECOVERY=65
...
NN_RETRAIN_INTERVAL_SEC      = 3600  # retrain NN every 1 hour (v18.92: 7200→3600 — faster regime adaptation; at WR=30.1% the model calibration drifts within 1-2h of market regime shifts; 1h retrains improve threshold EMA convergence speed by 2×; min 50 new outcomes still required so retrain only fires after adequate new data)
...
class UnitySignalFilter:
    ...
    def apply(
        ...
        if nn_trainer is not None and self._health.is_available("NeuralNetwork"):
            ...
            try:
                ...
                if _g4_drought > 1800:  # >30min drought
                    _g4_relaxed = max(NN_WIN_PROB_GATE - 0.05, nn_threshold - 0.02)  # v18.89: 0.35→NN_WIN_PROB_GATE-0.05=0.40 — RECALIBRATION FIX: floor 0.35 was set when base=0.39 (0.35=0.39-0.04); with base now=0.45 the old 0.35 allowed crisis+drought to drop threshold to max(0.35,0.46-0.02)=0.44 which is BELOW base=0.45 (counterproductive: crisis gate should be at or above base, not below it); new floor=0.45-0.05=0.40 means drought can relax crisis down to max(0.40,0.53-0.02)=0.51 — still above base; non-crisis drought: max(0.40,0.45-0.02)=0.43 (was 0.43 with old floor, unchanged) [v18.89]
        ...
        if _multiflip_count == 3:
            ...
            self._logger.debug(
                f"[G9-FlipFloor v18.96] {symbol} 3/3 FLIP ZONE → G9 floor +3pts "
    ...
    def _update_threshold_rl(self):
        ...
        try:
            ...
            if _staleness > _decay_start * 2.0 and _wr_for_decay < 0.40:
                _drought_cut = min(6.0, (_staleness - _decay_start * 2.0) / 120.0)
                if _drought_cut > 0.5:
                    self._logger.info(
                        f"🌵 [Apex-#7 v18.85] Drought base-cut: stale={_staleness/60:.1f}min "
                        f"WR={_wr_for_decay:.0%} → base -{_drought_cut:.1f}% [v18.85]"
                    )
        except Exception:
            _drought_cut = 0.0
...
class UnityEngine:
    ...
    def _print_startup_banner(self):
        ...
        if _intel_tier != "SOVEREIGN":
            logger.info(
                "   ℹ️  On Railway: nixpacks.toml installs torch==2.4.0+cpu at build time → "
    ...
    async def run(self) -> bool:
        ...
        # Increments scan_cycles every 30s independently of the bot scanner,
        # guaranteeing the watchdog (1800s) never fires unless the event loop
        # is genuinely dead.  Complements the bot's own scan_cycles increment.
        ...
        async def _scan_cycle_heartbeat_task() -> None:
            ...
            _hb_log.info("💓 [v19.0] Scan-cycle heartbeat started (30s interval, watchdog guardian)")
            while True:
                try:
                    await asyncio.sleep(30.0)
                    self.metrics.scan_cycles += 1
                    _hb_log.debug(f"💓 Heartbeat: scan_cycles={self.metrics.scan_cycles}")
                except asyncio.CancelledError:
                    _hb_log.info("Scan-cycle heartbeat cancelled")
                    break
        ...
        self._logger.info("✅ [v19.0] Scan-cycle heartbeat started — stale/zombie restart prevention")
