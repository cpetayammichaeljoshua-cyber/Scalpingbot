...
class DynamicBacktester:
    ...
    def quality_bias(self, symbol: str, *, max_age_sec: int = 7200) -> float:
        """
        Map per-symbol backtest into a [-8.0, +5.0] quality-score bias.
        ...
        if r.win_rate >= 0.45 and r.profit_factor >= 1.25 and r.ev_r > 0.03:
            return +5.0
        if r.win_rate >= 0.38 and r.profit_factor >= 1.05 and r.ev_r > 0.0:
            return +2.0
        if r.win_rate < 0.28 or r.profit_factor < 0.75 or r.ev_r < -0.12:
            return -8.0
        if r.win_rate < 0.35 or r.profit_factor < 0.95:
            return -3.0
        return 0.0
        ...
        while True:
            ...
            try:
                ...
                _LOG.info(
                    ...
                    len(self._results), ok, strong, good, bad, top3_str,
                )
            except Exception as e:    # pragma: no cover
