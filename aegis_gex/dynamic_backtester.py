"""
aegis_gex.dynamic_backtester — v9.9.1 Apex-#5
=============================================
...
def _vectorized_backtest(
    ...
    *,
    ...
    # ── v18.11: Backtesting Overfitting Probability (PBO) ─────────────────────
    # Apply Walk-Forward, Bootstrap PBO, and Deflated Sharpe Ratio analysis.
    # Provides a quality penalty (-0 to -5 pts) that integrates into Gate 8.5.
    # Only fires when ≥ 20 trades are present — harmless no-op otherwise.
    pbo_score = 0.0
    wfr       = 1.0
    dsr       = 0.0
    pbo_label = "CLEAN"
    pbo_pen   = 0.0
    if _PBO_ANALYZER is not None and len(r_returns) >= 20:
        try:
