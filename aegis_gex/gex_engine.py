...
class AEGISGEXEngine:
    ...
    async def compute_gex_snapshot(
        ...
        try:
            ...
            # Resolve klines caller — BTCUSDTTrader uses (interval, limit, symbol)
            # while the AEGIS-standalone client uses (symbol, timeframe, limit).
            # Detect by inspecting the first parameter name via __code__ / signature.
            _kl_fn = getattr(client, "get_klines", None)
            if _kl_fn is not None:
                import inspect as _ins
                _params = list(_ins.signature(_kl_fn).parameters.keys())
                # BTCUSDTTrader: (interval, limit, symbol); use keyword args to be safe
                if _params and _params[0] in ("interval", "self"):
                    _klines_coro = client.get_klines(timeframe, limit, symbol)
                else:
                    _klines_coro = client.get_klines(symbol, timeframe, limit)
            else:
                return None

            # get_premium_index may not exist on all client types — fall back to
            # get_funding_rate which hits the same /fapi/v1/premiumIndex endpoint.
            _prem_fn = getattr(client, "get_premium_index", None)
            if _prem_fn is None:
                _prem_fn = getattr(client, "get_funding_rate", None)
            _prem_coro = _prem_fn(symbol) if _prem_fn else asyncio.sleep(0)
