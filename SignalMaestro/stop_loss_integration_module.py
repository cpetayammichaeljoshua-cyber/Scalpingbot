...
class StopLossIntegrator:
    ...
    async def execute_stop_loss_action(self, action: StopLossAction) -> bool:
        ...
        try:
            ...
            # Execute partial closure via Unity if available
            if hasattr(self.bot, 'unity') and self.bot.unity:
                unity_success = await self._execute_unity_partial_close(action)
                if not unity_success:
                    self.logger.warning(f"⚠️ Unity partial close failed for {action.symbol}")
    ...
    async def _execute_unity_partial_close(self, action: StopLossAction) -> bool:
        """Execute partial position closure via Unity with proper TP/SL logic"""
        try:
            if hasattr(self.bot, 'unity') and self.bot.unity:
                unity = self.bot.unity
                ...
                if action.level in ['TP1', 'TP2']:
                    ...
                    if hasattr(unity, 'partial_take_profit'):
                        tp_result = await unity.partial_take_profit(
                            action.symbol,
                            tp_level,
                        ...
                        if tp_result.get('success', False):
                            self.logger.info(f"✅ Unity partial TP executed: {action.symbol} {action.level} ({action.percentage}%)")
                            
                            # Now move the stop loss
                            await self._execute_unity_sl_move(action, tp_level)
                            return True
                        else:
                            self.logger.warning(f"⚠️ Unity partial TP failed: {tp_result.get('error')}")
                            return False
                    else:
                        # Fallback to regular close_position
                        return await self._execute_unity_regular_close(action)
                        
                elif action.level == 'TP3':
                    # Final TP - close entire remaining position
                    if hasattr(unity, 'close_position'):
                        result = await unity.close_position(
                            action.symbol,
                        ...
                        if success:
                            self.logger.info(f"🏆 Unity TP3 full closure executed: {action.symbol}")
                        return success
                    else:
                        return await self._execute_unity_regular_close(action)
                    ...
                    if hasattr(unity, 'close_position'):
                        ...
                        success = result.get('success', False) if isinstance(result, dict) else bool(result)
                        if success:
                            self.logger.warning(f"🛑 Unity SL closure executed: {action.symbol}")
                        return success
                    else:
                        return await self._execute_unity_regular_close(action)
                ...
                self.logger.warning(f"⚠️ Unity integration not available")
                return False
                
        except Exception as e:
            self.logger.error(f"❌ Unity execution error: {e}")
            return False
    
    async def _execute_unity_sl_move(self, action: StopLossAction, tp_level: int) -> bool:
        ...
        try:
            ...
            if action.symbol in self.stop_loss_managers:
                ...
                if new_sl_price and hasattr(unity, 'update_stop_loss'):
                    ...
                    sl_result = await unity.update_stop_loss(
                        action.symbol,
                        new_sl_price,
                        sl_reason
                    )
                    ...
                    if success:
                        self.logger.info(f"✅ Unity SL moved: {action.symbol} -> {new_sl_price:.6f}")
                    else:
                        self.logger.warning(f"⚠️ Unity SL move failed: {sl_result.get('error')}")
                    return success
                else:
    ...
    async def _execute_unity_regular_close(self, action: StopLossAction) -> bool:
        """Fallback to regular close_position method"""
        try:
            if hasattr(self.bot.unity, 'close_position'):
                result = await self.bot.unity.close_position(
                    action.symbol,
                ...
                self.logger.warning(f"⚠️ Unity close_position method not available")
                return False
        except Exception as e:
            self.logger.error(f"❌ Unity regular close error: {e}")
            return False
