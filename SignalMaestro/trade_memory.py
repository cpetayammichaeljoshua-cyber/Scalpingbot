...
class TradeMemory:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        Fix: use the unambiguous outcome field:
          ...
          losses = outcome='SL' OR (EXPIRED AND pnl<=-0.5) — definitive loss (same as
                   _is_definitive_loss in OutcomeTracker and build_label() ±0.5% threshold)
          EXPIRED neutral trades are reported separately (not counted as win or loss).

        [v15.3 Bug M FIX]: Previously losses = outcome='SL' only, excluding 101 EXPIRED
        trades with pnl_pct<=-0.5% that ARE counted as definitive losses by the RL/Bayes
        system.  This caused heartbeat WR=33.1% (inflated) vs true WR=31.6%.  Now aligned
        with _is_definitive_loss and build_label() ±0.5% threshold used throughout.
        ...
        with self._db() as c:
            ...
            row = c.execute("""
                SELECT
                    ...
                    COUNT(CASE WHEN outcome = 'SL'
                               OR  (outcome = 'EXPIRED' AND pnl_pct <= -0.5)
    ...
    def get_symbol_pnl_history(self, symbol: str, limit: int = 60) -> list:
        """
        Return up to `limit` recent resolved trades for `symbol` as a list of
        pnl_pct floats, oldest→newest.  Used by the Sortino-based blacklist
        filter (v9.9 Apex-#5) so symbols with high payoff but moderate WR
        aren't unfairly killed by the WR-only filter.

        Only definitively-resolved trades are returned (TP1/TP2/TP3/SL,
        plus EXPIRED with |pnl| ≥ 0.5 % to align with build_label thresholds).
        """
        try:
            with self._db() as c:
                rows = c.execute("""
                    ...
                    ORDER BY COALESCE(outcome_timestamp, timestamp) DESC
                    LIMIT ?
                """, (symbol, int(max(1, limit) * 2))).fetchall()
        except Exception:
            return []
        out: list = []
        for outcome, pnl in rows:
            ...
            try:
                ...
                continue
            if outcome in ("TP1", "TP2", "TP3", "SL"):
                out.append(pnl_v)
            elif outcome == "EXPIRED" and abs(pnl_v) >= 0.5:
                out.append(pnl_v)
            if len(out) >= limit:
                break
    ...
    async def _check_all(self):
        ...
        for trade in open_trades:
            ...
            if should_resolve:
                ...
                _is_definitive_win  = resolved_outcome in ("TP1", "TP2", "TP3")
                _is_definitive_loss = (resolved_outcome == "SL") or (
                    resolved_outcome == "EXPIRED" and pnl_pct <= -0.5
                )
                if self.bot is not None and hasattr(self.bot, "update_loss_streak"):
                    is_loss = _is_definitive_loss
                    try:
                ...
                # ── Unity Engine: update RL booster + global metrics ──────────
                if self.bot is not None and (_is_definitive_win or _is_definitive_loss):
                    _u_booster     = getattr(self.bot, "_unity_booster",      None)
                    _u_metrics     = getattr(self.bot, "_unity_metrics",      None)
                    _u_sym_tracker = getattr(self.bot, "_unity_sym_tracker",  None)
                    try:
                        if _u_booster is not None:
                            _u_booster.record_outcome(_is_definitive_win, pnl_pct)
                        if _u_metrics is not None:
                            if _is_definitive_win:
                                _u_metrics.win_count += 1
                            else:
                                _u_metrics.loss_count += 1
                            _u_metrics.total_profit_pct += pnl_pct
                        # FIX: Update PerSymbolTracker so Gate 8 (per-symbol win rate)
                        # actually has data to work with.  Previously this was never called,
                        # leaving the tracker empty and Gate 8 permanently inactive.
                        if _u_sym_tracker is not None and symbol:
                            _u_sym_tracker.record(symbol, _is_definitive_win, pnl_pct)
                    except Exception as _ue:
                        self.logger.debug(f"Unity metrics update skipped: {_ue}")
