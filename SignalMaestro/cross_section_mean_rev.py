...
CSM_Z_EXTREME     = 2.0     # standard deviations → extreme zone
CSM_Z_WEAK        = 1.0     # |z| < this → no cross-sectional edge
CSM_TOP_PCT       = 0.03    # top 3% overbought → short candidates
CSM_BOT_PCT       = 0.03    # bottom 3% oversold → long candidates
CSM_QUALITY_BOOST = 5.0     # quality pts for extreme cross-sectional signal
CSM_NEUTRAL_PENALT = -2.0   # quality pts when symbol in the neutral middle
CSM_RECOMPUTE_N   = 5       # recompute every N price updates
CSM_MIN_SYMBOLS   = 15      # minimum universe size for reliable z-scores
