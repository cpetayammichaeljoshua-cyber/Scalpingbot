...
class Database:
    ...
    async def save_signal_log(self, signal_id: int, payload: Dict[str, Any], 
                             response: Dict[str, Any], status: str):
        """Save Signal dispatch log"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute("""
                    INSERT INTO signal_logs (signal_id, payload, response, status)
                    VALUES (?, ?, ?, ?)
                """, (signal_id, json.dumps(payload), json.dumps(response), status))
                await db.commit()
                
        except Exception as e:
            self.logger.error(f"Error saving Unity log: {e}")
            raise
    ...
    async def cleanup_old_data(self, cutoff_timestamp: float):
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                # Clean old Unity logs
                await db.execute(
                    "DELETE FROM signal_logs WHERE created_at < ?",
                    (cutoff_date.isoformat(),)
                )
    ...
    async def test_connection(self) -> bool:
        ...
        except (sqlite3.Error, OSError, ValueError, AttributeError, KeyError):
            return False

    def save_signal_data(self, data: Dict[str, Any]) -> Optional[int]:
        """Save signal data to database (kept synchronous - enhanced_perfect_scalping_bot_v3 calls it without await)"""
        try:
