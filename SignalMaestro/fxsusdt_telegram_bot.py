...
class FXSUSDTTelegramBot:
    ...
    def __init__(self):
        ...
        # ── Cornix-replacement inline-keyboard menu (raw-mode integration
        #    with the engine's custom long-poller — no PTB Application required) ──
        self.cornix_menu = None
        try:
            from SignalMaestro.cornix_menu_bot import CornixMenuBot

            async def _balance_provider(cfg):
                try:
                    if hasattr(self, "trader") and self.trader and hasattr(self.trader, "get_account_balance"):
                        bal = await self.trader.get_account_balance()
                        if isinstance(bal, dict):
                            return {
                                "balance":   float(bal.get("balance", bal.get("total", 0))),
                                "available": float(bal.get("available", bal.get("free", 0))),
                                "equity":    float(bal.get("equity", bal.get("total", 0))),
                                "unrealised_pnl": float(bal.get("unrealised_pnl", bal.get("unrealizedPnl", 0))),
                            }
                        return {"balance": float(bal), "available": float(bal), "equity": float(bal), "unrealised_pnl": 0.0}
                except Exception:
                    pass
                return {"balance": 0.0, "available": 0.0, "equity": 0.0, "unrealised_pnl": 0.0}

            async def _positions_provider(cfg):
                try:
                    if hasattr(self, "trader") and self.trader and hasattr(self.trader, "get_open_positions"):
                        return await self.trader.get_open_positions() or []
                except Exception:
                    pass
                return []

            async def _history_provider(cfg):
                try:
                    if hasattr(self, "trader") and self.trader and hasattr(self.trader, "get_trade_history"):
                        return await self.trader.get_trade_history(limit=50) or []
                except Exception:
                    pass
                return []

            async def _signal_executor(user_id, signal_id, cfg, retry: bool = False):
                if hasattr(self, "execute_signal"):
                    try:
                        return await self.execute_signal(signal_id, user_cfg=cfg, retry=retry)
                    except TypeError:
                        try:
                            return await self.execute_signal(signal_id)
                        except Exception as e:
                            return f"executor error: {e}"
                return f"queued (mode={cfg.trading_mode})"

            async def _shared_session():
                # Reuse the bot's existing aiohttp session for raw Telegram calls
                return await self._get_tg_session()

            # Build admin_ids list from env + admin_chat_id
            _raw_admin = os.getenv("UNITY_ADMIN_IDS", "") or os.getenv("ADMIN_CHAT_ID", "")
            _admin_ids = [int(x.strip()) for x in _raw_admin.replace(";", ",").split(",")
                          if x.strip().lstrip("-").isdigit()]
            # Also include self.admin_chat_id if it's a numeric user/chat
            if self.admin_chat_id and str(self.admin_chat_id).lstrip("-").isdigit():
                _aid = int(self.admin_chat_id)
                if _aid not in _admin_ids:
                    _admin_ids.append(_aid)
            self.cornix_menu = CornixMenuBot(
                balance_provider=_balance_provider,
                positions_provider=_positions_provider,
                history_provider=_history_provider,
                signal_executor=_signal_executor,
                bot_token=self.bot_token,
                session_provider=_shared_session,
                admin_ids=_admin_ids or [],
            )
            _gate_str = f"{len(_admin_ids)} admin(s)" if _admin_ids else "open (all users)"
            self.logger.info(f"✅ Cornix-replacement menu system attached (inline keyboards · 8 exchanges · encrypted vault · admin gate: {_gate_str})")
        except Exception as _menu_err:
            self.logger.warning(f"⚠️ Cornix menu init failed (slash commands still work): {_menu_err}")
    ...
    async def send_signal_to_channel(self, signal: SwarmSignal,
        ...
        try:
            ...
            if channel_ok:
                ...
                # Generate a unique signal_id and push to the cornix cache
                try:
                    _sig_id = f"{symbol}_{signal.action}_{int(now_ts)}"
                    _cornix_menu = getattr(self, "cornix_menu", None)
                    if _cornix_menu and callable(getattr(_cornix_menu, "cache_signal", None)):
                        _cornix_menu.cache_signal(_sig_id, {
                            "symbol":    symbol,
                            "direction": "LONG" if signal.action == "BUY" else "SHORT",
                    ...
                    # v10.5: Send admin a private copy with Execute/Ignore buttons
                    if self.admin_chat_id and str(self.admin_chat_id) != str(self.signal_channel_id):
                        _keys_task = getattr(_cornix_menu, "store", None)
                        try:
                            from SignalMaestro.cornix_menu_bot import signal_action_kb
                            _admin_kb = signal_action_kb(_sig_id, has_keys=True)
                            _dir  = "LONG" if signal.action == "BUY" else "SHORT"
                            _emj  = "🟢" if signal.action == "BUY" else "🔴"
                            _pri  = (
                                f"{_emj} *{symbol} {_dir}* @ `{signal.entry_price:.4g}`\n"
                                f"TP1: `{signal.take_profit_1:.4g}` | SL: `{signal.stop_loss:.4g}`\n"
                            ...
                            self.logger.debug(f"Admin signal_action_kb error: {_kb_err}")
    ...
    async def process_signals(self, signals: List[SwarmSignal]) -> bool:
        ...
        if not _ai_ready:
            ...
            if not _consensus_bypass:
                ...
                self.logger.info(
                    ...
                    f"(consensus={_sig_consensus:.0%} < 95% or "
            ...
            self.logger.info(
                ...
                f"LLMs rate-limited but consensus={_sig_consensus:.0%}≥95% "
                f"+ conf={signal.confidence:.1f}%+{_AI_GATE_MAX_BOOST:.0f}boost"
                f"≥{confidence_threshold:.0f}% qualifies. "
                f"G0DM0D3 is 6% of swarm weighting; 9-agent unanimous rule-based "
                f"consensus overrides AI gate requirement. [v8.1 pre-boost fix]"
            )
        ...
        if self.nn_trainer and getattr(self.nn_trainer, "trained", False) and _nn_accuracy >= 0.55:
            try:
                ...
                if nn_win_prob < _absolute_floor:
                ...
                elif _is_unanimous and nn_win_prob >= 0.40 and not (_high_uncertainty and nn_uncertainty > 0.20):
                    # v8.4: Unanimous swarm bypass floor raised 0.28 → 0.40.
                    # At win_prob=35%, even unanimous consensus produces a losing
                    # strategy (Kelly says ~0%–2% position — effectively skip it).
                    # Requiring 40%+ ensures EV is positive at realistic 2:1 R:R.
                    ...
                    self.logger.info(
                        ...
                        f"win_prob={nn_win_prob:.0%} σ={nn_uncertainty:.2f} "
                        f"→ conf -{_override_penalty:.1f}pt (Unity G4 ≥0.55 still authoritative) [v9.5]"
                    )
                else:
        ...
        if _unity_filter is not None and _unity_booster is not None:
            try:
                ...
                _unity_filter.mark_signal_sent(symbol)
                self.logger.info(
        ...
        try:
            ...
            async with session.get(
                ...
                for update in updates:
                    ...
                    # ── Cornix-replacement inline-keyboard menu — handles
                    #    callback_query (button presses), /start, /menu, and
                    #    free-text wizard input.  Returns True if it consumed
                    #    the update so the slash-command path is skipped.
                    cornix = getattr(self, "cornix_menu", None)
                    if cornix is not None:
                        try:
                            if await cornix.process_raw_update(update):
                                continue
                        except Exception as menu_err:
                            self.logger.debug(f"Cornix menu raw dispatch error: {menu_err}")
...
• BTC volatile — use wider SL at high leverage"""
    ...
    async def start_telegram_polling(self):
        ...
        try:
            ...
            # Register all commands dynamically (skip /start — handled by Cornix menu)
            ...
            # ── Attach Cornix-replacement inline-keyboard menu system ────
            try:
                from SignalMaestro.cornix_menu_bot import CornixMenuBot
                ...
                async def _balance_provider(cfg):
                    try:
                        ...
                        self.logger.debug(f"balance_provider: {e}")
                    return {"balance": 0.0, "available": 0.0, "equity": 0.0, "unrealised_pnl": 0.0}

                async def _positions_provider(cfg):
                    try:
                        if hasattr(self, "trader") and self.trader and hasattr(self.trader, "get_open_positions"):
                            pos = await self.trader.get_open_positions()
                            return pos or []
                    except Exception as e:
                        self.logger.debug(f"positions_provider: {e}")
                    return []
                ...
                async def _history_provider(cfg):
                    try:
                        ...
                        self.logger.debug(f"history_provider: {e}")
                    return []
                ...
                async def _signal_executor(user_id, signal_id, cfg, retry: bool = False):
                    if hasattr(self, "execute_signal"):
                        try:
                            ...
                            return await self.execute_signal(signal_id)
                    return f"signal {signal_id} queued (mode={cfg.trading_mode})"
                ...
                self.cornix_menu.attach(application)
                self.logger.info("✅ Cornix-replacement menu system attached (inline keyboards, 8 exchanges, encrypted vault)")
            except Exception as menu_err:
                self.logger.warning(f"⚠️ Cornix menu attach failed (slash commands still active): {menu_err}")
