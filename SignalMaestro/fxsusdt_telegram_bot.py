...
class FXSUSDTTelegramBot:
    ...
    def __init__(self):
        ...
        self._EV_FAIL_ESCALATE_N2: int = 7             # level-3 after this many consecutive fails [v18.71: 6→7 — BNBUSDT reached streak=25 so threshold doesn't affect chronic offenders; raising to 7 prevents premature 20-min lock-out for symbols that had a brief EV-geometry shift (e.g. ATR spike → spread widens → EV dips for 6 cycles then recovers); requires one more fail before the harsh suppress fires]
        self._EV_FAIL_VERY_LONG_SEC: float = 1200.0    # 20-min suppress for chronic EV-fails [v18.71: 1800→1200 — 30 min is 3× a full scan cycle; structural EV deficits typically self-heal within 15-20 min as candle geometry resets; 1200s still prevents chronic offenders from burning swarm capacity while allowing re-evaluation at a new session boundary sooner]
    ...
    def _refresh_symbol_blacklist(self):
        ...
        try:
            ...
            PROTECTED_SYMBOLS = {
                ...
                # NEARUSDT removed (v20.5): WR=8%/25 trades < 15% floor; treating as normal symbol
    ...
    async def process_signals(self, signals: List[SwarmSignal]) -> bool:
        ...
        # ── Phase 1.9: StochasticQuant calibration + NN feature stamp (v19.1) ─
        # Awaited calibration of OU/Heston/JumpDiff/Kalman for this symbol using
        # the klines already fetched in Phase 1 PM framework.
        # v19.1 UPGRADE from fire-and-forget (ensure_future) to awaited call so:
        #   1. G8.5sq gate gets warm cache guaranteed for this cycle (not next)
        #   2. QuantSignal values are stamped on `signal` via setattr for NN features
        #      (nn_trainer._extract_features uses getattr(signal, attr, default))
        # Latency: ~1ms for cached TTL hit; ~50-100ms for recalibration (in thread pool)
        # both are negligible vs the 50-200ms NN inference that follows.
        _sq_cal_eng = getattr(self, "_unity_stochastic_quant", None)
        if _sq_cal_eng is not None and _pm_klines and len(_pm_klines) >= 20:
            try:
                import numpy as _np_sq19
                _sq_closes_raw = [float(k[4]) for k in _pm_klines[-100:]]
                if len(_sq_closes_raw) >= 10:
                    _sq_prices_cal  = _np_sq19.array(_sq_closes_raw, dtype=float)
                    _sq_returns_cal = _np_sq19.diff(
                        _np_sq19.log(_np_sq19.maximum(_sq_prices_cal, 1e-9))
                    )
                    # Await calibration so cache is warm before Phase 2 NN + G8.5sq
                    _sq_result = await _sq_cal_eng.get_signal(
                        symbol, _sq_prices_cal, _sq_returns_cal
                    )
                    # Stamp QuantSignal fields on signal for NN feature consumption
                    if _sq_result is not None:
                        object.__setattr__(signal, "ou_zscore",      float(_sq_result.ou_zscore))
                        object.__setattr__(signal, "ou_regime",      str(_sq_result.ou_regime))
                        object.__setattr__(signal, "heston_vol",     float(_sq_result.heston_vol))
                        object.__setattr__(signal, "heston_regime",  str(_sq_result.heston_regime))
                        object.__setattr__(signal, "jump_prob_1d",   float(_sq_result.jump_prob_1d))
                        object.__setattr__(signal, "kalman_trend",   float(_sq_result.kalman_trend))
        ...
        if self.nn_trainer and getattr(self.nn_trainer, "trained", False) and _nn_accuracy >= 0.55:
            try:
                ...
                # the gate entirely.
                #
                # v22.0: NN Pessimism Correction — the NN may enter a "learned
                # pessimism" state when trained on a losing-dominant dataset (WR<35%).
                # At WR=29.6%, the "always-predict-loss" naive baseline achieves
                # ~70.4% accuracy; if the NN's actual accuracy is only marginally
                # above this, it has NOT learned genuine signal discrimination and is
                # instead predicting losses for nearly all inputs.  In this regime
                # the 9-agent swarm consensus is more reliable than the NN.
                # Correction: linearly reduce the absolute floor by up to 7pp as the
                # NN's edge over the naive baseline shrinks toward zero.
                #   edge=0pp  (zero learning beyond baseline): floor 0.20→0.13 (-7pp)
                #   edge=5pp  (moderate learning):             floor 0.20→0.165 (-3.5pp)
                #   edge≥10pp (solid discrimination):          no reduction (floor=0.20)
                _nn_accuracy     = getattr(self.nn_trainer, "last_accuracy", 0.0)
                _ub_pess         = getattr(self, "_unity_booster", None)
                _global_wr_pess  = 0.30  # pessimistic default
                if _ub_pess is not None:
                    _ba_pess = float(getattr(_ub_pess, "_bayes_alpha", 2.0) or 2.0)
                    _bb_pess = float(getattr(_ub_pess, "_bayes_beta",  2.0) or 2.0)
                    _global_wr_pess = _ba_pess / max(1.0, _ba_pess + _bb_pess)
                _nn_baseline_acc  = 1.0 - _global_wr_pess  # naive always-predict-loss acc
                _nn_edge_pess     = max(0.0, _nn_accuracy - _nn_baseline_acc)
                _floor_reduction  = max(0.0, 0.07 * (1.0 - min(1.0, _nn_edge_pess / 0.10)))
                _absolute_floor   = max(0.10, _reject_thresh - 0.18 - _floor_reduction)
                if _floor_reduction >= 0.01:
                    self.logger.debug(
                        f"🧠 [NN-Pessimism v22.0] {symbol}: nn_acc={_nn_accuracy:.1%} "
                        f"baseline={_nn_baseline_acc:.1%} edge={_nn_edge_pess:.1%} "
                        f"→ floor_reduction={_floor_reduction:.3f} "
                        f"(floor {_reject_thresh - 0.18:.2f}→{_absolute_floor:.2f})"
                    )
                #
                ...
                if nn_win_prob < _absolute_floor:
                    ...
                    if _is_unanimous and _high_model_uncertainty and nn_win_prob >= _UNC_OVERRIDE_MIN_WIN_PROB:
                        ...
                        # v22.0: Pessimism-aware cooldown — when NN is in pessimism mode
                        # (_floor_reduction>0.02), the 10-min cooldown over-suppresses:
                        # market conditions CAN shift enough in 3 min to warrant re-eval.
                        # Reduce to 3 min when pessimism correction is active.
                        _abs_reject_cd = 180 if _floor_reduction > 0.02 else 600
                        self._nn_reject_cooldown[symbol] = time.time() + _abs_reject_cd
                        self.logger.info(
                            f"⏳ [{symbol}] NN ABSOLUTE REJECT → {_abs_reject_cd//60}min cooldown "
                            f"(win_prob={nn_win_prob:.0%} < {_absolute_floor:.0%} floor"
                            f"{', pessimism-mode' if _floor_reduction > 0.02 else ''}) [v22.0]"
                        )
                        return False
                ...
                if not _passed:
                    ...
                    if _reason.startswith("G0_FAIL"):
                        ...
                        if _streak >= self._EV_FAIL_ESCALATE_N2:
                            ...
                            _cd      = min(self._EV_FAIL_VERY_LONG_SEC, 600.0) if _is_asian \
                                       else self._EV_FAIL_VERY_LONG_SEC
                            _sess_tag = "Asian-10min-cap" if _is_asian else "20min"  # v18.71: 900→600s Asian cap; 1800→1200s other; BNBUSDT was suppressed 900s; London pre-market EV recovery typically arrives within 10min of Asian close
                            self.logger.info(
