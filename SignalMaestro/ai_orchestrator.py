...
class AIOrchestrator:
    ...
    def __init__(self, enforce_requirements: bool = False):
        ...
        if SENTIMENT_ANALYZER_AVAILABLE and self._is_component_available('sentiment_analysis'):
            ...
            self.logger.info("✅ Sentiment Analyzer running in SOVEREIGN SKLEARN mode (score=1.00) [v18.61]")
        else:
        ...
        if MARKET_PREDICTOR_AVAILABLE and self._is_component_available('market_prediction'):
            ...
            self.logger.info("✅ Market Predictor running in SOVEREIGN SKLEARN mode (score=1.00) [v18.61]")
        else:
    ...
    def _log_initialization_summary(self):
        ...
        try:
            ...
            for component, result in self.system_capability.components.items():
                _sovereign = (result.level == CapabilityLevel.FULL) if hasattr(result, 'level') and hasattr(CapabilityLevel, 'FULL') else False
                if result.available:
                    self.logger.info(f"✅ {component}: Active (intelligence: {result.intelligence_score:.2f})")
                elif _sovereign:
                    self.logger.info(f"✅ {component}: SOVEREIGN SKLEARN mode (intelligence: {result.intelligence_score:.2f}) [v18.61]")
