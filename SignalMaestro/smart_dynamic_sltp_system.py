...
class SmartDynamicSLTPSystem:
    ...
    def _calculate_atr(self, high: np.ndarray, low: np.ndarray,
        ...
        """
        Calculate Average True Range using Wilder's smoothing (proper ATR).
        Wilder's ATR uses EMA-style smoothing: ATR_n = (ATR_{n-1}*(p-1) + TR_n) / p
        This responds faster to volatility changes than the old SMA approach,
        giving more accurate SL/TP placement during regime shifts.
        """
        try:
            ...
            # Seed with SMA of first `period` bars, then Wilder-smooth the rest
    ...
    def _calculate_adx(self, high: np.ndarray, low: np.ndarray,
        ...
        Uses proper EMA-based smoothing of +DM, -DM, and TR to avoid lag.
        """
        try:
            if len(high) < period + 2:
                return 20.0
            ...
            def _wilder_smooth(arr: np.ndarray, n: int) -> float:
                if len(arr) < n:
                    return float(np.mean(arr)) if len(arr) > 0 else 0.0
                val = float(np.sum(arr[:n]))
                for i in range(n, len(arr)):
                    val = val - (val / n) + float(arr[i])
                return val
