...
class AIOrchestrationAgent:
    ...
    async def _query_openai(self, prompt: str) -> Optional[dict]:
        ...
        try:
            ...
            if any(p in err_str for p in _PERM_PHRASES):
                # v17.4: 401 → 30-min temporary backoff, not permanent disable.
                # Previously 401 set _openai_perm_disabled=True causing a 4-hour
                # lockout even for transient auth rejections (key rotation, network
                # glitch, momentary Binance-side auth hiccup).  Now 401 is treated
                # identically to quota/rate-limit: 30-min backoff with auto-retry.
                # The engine already has OpenRouter/Claude as primary; this ensures
                # OpenAI re-enters the rotation every 30 min without operator action.
                _cooldown_401 = 1800  # 30-min re-probe interval
                _was_enabled  = self._openai_disabled_until == 0.0
                self._openai_disabled_until = time.time() + _cooldown_401
                if _was_enabled:
                    self.logger.info(
                        "ℹ️  OpenAI auth (401) — 30-min backoff, auto-retry enabled. "
                        "Primary AI is OpenRouter (active). Set OPENAI_API_KEY to restore tertiary fallback."
                    )
                return None
    ...
    async def analyze(self, symbol: str, closes: List[float],
        ...
        if data is None:
            ...
            if data:
                ...
                # NOTE: bare Claude slugs use the Anthropic direct-API, NOT OpenRouter.
                # Do NOT record their failures to SmartLLMRouter — that would trigger
                # a false quarantine + free-tier-fast-path for OpenRouter paid models.
                ...
                # NOTE: bare OpenAI slug (gpt-4o-mini) is for direct OpenAI API, NOT OpenRouter.
                # Do NOT record OpenAI auth failures to SmartLLMRouter to avoid false quarantine.
...
class MiroFishSwarmStrategy:
    ...
    def __init__(self):
        ...
        self.timeframes     = ["15m", "5m"]   # v15.2: added 5m — doubles signal opportunities; same 14-gate filter applies to both TFs; 5m finds faster-moving setups that don't show on 15m
        self.primary_timeframe = "15m"

        # Pre-boost signal gates — v15.2: relaxed to restore signal flow (23% WR → starvation)
        self.min_signal_strength = 60.0     # v15.2: 65→60 (was blocking all signals at 23% WR)
        self.min_confidence      = 62.0     # v15.2: 67→62 (agents score 60-67 at low WR)
        self.min_swarm_consensus = 0.92     # v15.2: 0.95→0.92 (allow 92%+ consensus signals)
        self.min_active_agents   = 7        # v15.2: 8→7 (quorum: 7/10 agents sufficient)
        ...
        self._global_win_rate  = 0.28   # v10.7: lowered 0.335→0.28 to reflect actual live WR
                                        # (~25% observed) so Kelly fractions are conservative
                                        # from session start; scanner syncs from trade_memory
                                        # every cycle once ≥20 resolved trades accumulate.
        ...
        try:
            ...
            try:
                ai_vote, ai_conf, ai_narrative, react_trace = await asyncio.wait_for(
                    ...
                    timeout=50.0  # v9 FIX: was 10.0 — G0DM0D3 needs 35s+ for full cascade; 10s caused 100% timeout→NEUTRAL
                )
            ...
            # v15.2: replaced hard ZERO-dissent with 1-dissenter tolerance.
            # Old gate blocked every signal with even 1 contrary agent causing starvation.
            if n_contrary >= 2:
                self.logger.debug(
                    f"⚠️ [{symbol}|{tf}] CONSENSUS GATE: {n_contrary} contrary agents "
                    f"({', '.join(n for n, _, _ in contrary_agents)}) — rejected [v15.2]"
                )
                return None
            elif n_contrary == 1:
                if consensus < 0.95 or n_aligned < 7:
                    self.logger.debug(
                        f"⚠️ [{symbol}|{tf}] CONSENSUS GATE: 1 dissenter, "
                        f"consensus={consensus:.2f} aligned={n_aligned} — rejected [v15.2]"
                    )
                    return None
                # 1 dissenter with strong consensus — allow, penalty applied below
                weighted_conf = max(weighted_conf - 3.0, 50.0)
            ...
            # strategy where Unity closes 50% at TP1 and 35% at TP2).  TP1-only R:R
            ...
            _hist_wr = getattr(self, '_global_win_rate', 0.28)  # v10.7: fallback matches init default
            ...
            # v10.8 BUG FIX: Blended TP reward for Kelly b matches the actual
            # partial-exit plan (45% TP1, 35% TP2, 20% TP3).  Previously used
            # TP1-only distance which underestimated expected reward → sub-optimal
            # leverage sizing (Kelly fraction too conservative on high-R:R setups).
            _kelly_b = (tp1_dist * 0.45 + tp2_dist * 0.35 + tp3_dist * 0.20) / max(_sl_dist_rr, 1e-10)
