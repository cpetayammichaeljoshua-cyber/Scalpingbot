...
- Unity integration with proper formatting
...
🌐 **Unity:** {'🟢 Ready' if self.unity.webhook_url else '🔴 Not configured'}
    ...
    async def process_trading_signal(self, signal: Dict[str, Any], update):
        ...
        try:
            ...
            if await self.send_rate_limited_message(update.effective_chat.id, confirmation_msg):
                # Forward to Unity
                await self.forward_to_executor(signal, trade_progress)
...
⚡ **Monitoring:** Active"""
    ...
    async def forward_to_executor(self, signal: Dict[str, Any], trade: TradeProgress) -> bool:
        """Forward signal to Unity with proper formatting"""
        try:
            # Create Unity-compatible payload
            unified_signal = {
                'symbol': trade.symbol,
            ...
            # Forward through enhanced Unity integration
            result = await self.unity.send_initial_signal(unified_signal)

            if result.get('success'):
                self.logger.info(f"✅ Signal forwarded to Unity: {trade.symbol}")
                return True
            else:
                self.logger.warning(f"⚠️ Unity forwarding failed: {result.get('error')}")
                return False

        except Exception as e:
            self.logger.error(f"❌ Error forwarding to Unity: {e}")
            return False
    ...
    async def handle_tp1_hit(self, symbol: str):
        ...
        try:
            ...
            await self.unity.update_stop_loss(symbol, trade.entry_price, "TP1 hit - SL moved to entry")
...
⚡ **Monitoring TP2...**"""
    ...
    async def handle_tp2_hit(self, symbol: str):
        ...
        try:
            ...
            # Send SL update to Unity
            await self.unity.update_stop_loss(symbol, trade.tp1, "TP2 hit - SL moved to TP1")
...
⚡ **Final target: TP3**"""
    ...
    async def handle_tp3_hit(self, symbol: str):
        ...
        try:
            ...
            # Send full closure to Unity
            await self.unity.close_position(symbol, "TP3 hit - Full profit target reached", 100)
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.unity.webhook_url,
                    json=update_payload,
                    headers={'Content-Type': 'application/json'},
                    timeout=10
                ) as response:
                    if response.status == 200:
                        self.logger.info(f"✅ SL update sent to Unity: {symbol}")
                    else:
                        self.logger.warning(f"⚠️ Unity SL update failed: {response.status}")

        except Exception as e:
            self.logger.error(f"❌ Error sending SL update to Unity: {e}")

    async def send_trade_closure_to_unity(self, symbol: str, reason: str):
        """Send trade closure to Unity"""
        try:
            closure_payload = {
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        self.logger.info(f"✅ Trade closure sent to Unity: {symbol}")
                    else:
                        self.logger.warning(f"⚠️ Unity closure failed: {response.status}")

        except Exception as e:
            self.logger.error(f"❌ Error sending closure to Unity: {e}")
