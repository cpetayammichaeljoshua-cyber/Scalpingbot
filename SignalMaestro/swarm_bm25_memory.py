...
class BM25MemoryBank:
    ...
    def __init__(self, role: str):
        ...
        self.bm25 = None
        self._dirty: bool = False   # lazy-rebuild flag

    @staticmethod
    def _tokenize(text: str) -> List[str]:
        """
        Financial-aware tokenizer.  Preserves:
        - Numbers with decimals and percentages  (50.5%, 0.015)
        - Uppercase crypto ticker symbols        (BTC, ETH, SOLUSDT)
        - Regular words (lowercased)
        Single-character tokens are discarded.
        """
        # Extract numbers (including decimals and trailing %)
        numbers = re.findall(r'\d+\.?\d*%?', text)
        # Extract words (preserve uppercase tickers, lowercase the rest)
        words   = re.findall(r'[A-Za-z]{2,}', text)
        tokens  = numbers + [w.lower() for w in words]
        return tokens

    def _rebuild_index(self):
        """Rebuild BM25 index from current document set."""
    ...
    def add_lesson(self, situation: str, recommendation: str, meta: dict = None):
        """Add a lesson and mark the index as dirty (lazy rebuild)."""
        ...
        # Mark dirty — index rebuilt lazily before next query, not here.
        # This avoids rebuilding N times when N lessons are added in a batch.
        self._dirty = True

    def flush(self):
        """Rebuild index now if dirty (call once after a batch of add_lesson calls)."""
        if self._dirty:
            self._rebuild_index()

    def get_memories(self, current_situation: str, n_matches: int = 2) -> List[dict]:
        if not self.documents or not _HAS_BM25:
            return []
        # Lazy rebuild: only rebuild once before first query after mutations
        if self._dirty:
            self._rebuild_index()
