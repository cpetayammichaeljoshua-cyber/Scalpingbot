...
KF_Z_ENTRY        = 2.5    # Z-score threshold for entry signal
KF_Z_EXIT         = 0.5    # Z-score threshold for exit
KF_Z_EXHAUST      = 3.5    # Z-score > this → spread exhaustion, reduce size
KF_WINDOW         = 60     # rolling window for Z-score normalisation
KF_QUALITY_BOOST  = 5.0    # quality pts when Z-score ≥ entry threshold
KF_QUALITY_PENALT = -3.0   # quality pts when Z-score in exhaustion zone
KF_KELLY_MULT     = 1.10   # Kelly multiplier for high-conviction pair setups

# Kalman filter process/observation noise
KF_VE    = 0.001   # observation noise variance (Q)
KF_DELTA = 1e-4    # state evolution noise (Wt variance)
