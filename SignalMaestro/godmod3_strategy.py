...
class G0DM0D3Engine:
    ...
    async def analyze(
        ...
        system_prompt = (
            ...
            "(e) portfolio MaxDD > 50% (catastrophic drawdown — survival mode, no new risk), "
            "(f) OFI z-score absolute value > 3.5 AND against signal direction (extreme institutional opposition), "
            ...
            "Volume_ratio 1.2-2.5 + directional OFI = institutional conviction (>2.5 = overextension/exhaustion trap). "
            "OFI z-score >1.5 aligned with direction = strong institutional flow confirmation (+8pp confidence). "
            "VPIN: >0.65 = toxic order flow (informed traders active, adverse selection risk) → reduce confidence 15pp. "
            ...
            "Volume exhaustion: volume_ratio >3.0 AND price stalling = reversal risk → NEUTRAL regardless of direction. "
            "Spread width: spread >0.1% of price = illiquid conditions → reduce confidence 10pp. Vote: BUY/SELL/NEUTRAL. "
            "[3-EV-SYNTHESIS] E[V] = P_win × 2.65 − (1 − P_win). REQUIRE P_win≥40% (conservative) → positive EV. "
            "Below 40% win probability OR regime vs flow disagreement → NEUTRAL. Default to NEUTRAL when uncertain. "
            "EMERGENCY DRAWDOWN OVERRIDE: if MaxDD > 40% — downgrade confidence by 20pp. MaxDD > 50% → force NEUTRAL. "
            "Confidence: 55-70%=marginal EV(minimum viable), 70-85%=confirmed edge, 85-95%=institutional conviction. "
            "NEVER output confidence above 70 unless BOTH regime AND flow confirm the same direction. "
            ...
            "Any two of three misaligned = reduce confidence by 20pp (and cap at 65). All three misaligned = NEUTRAL. "
            "Momentum persistence: 3+ consecutive aligned candles upgrades confidence by 8pp (cap 90, not 95). "
            "IRONS quality floor: if signal quality score < 68 (crisis floor active) → cap confidence at 72. "
            "Funding cascade risk: |funding_rate| > 0.05% = imminent cascade → reduce confidence 15pp regardless of direction. "
            "Sharpe recovery signal: if Sharpe was <-1.0 last cycle but is now >0.0, regime is healing — +5pp bonus. "
            "Regime persistence: if HMM state unchanged for 3+ cycles, trend is structural not noise — +5pp conviction. "
            "STRICTLY output ONLY valid JSON — no commentary, no preamble: "
            "{\"vote\": \"BUY|SELL|NEUTRAL\", \"confidence\": 55-95, \"narrative\": \"≤120 char EV+regime+flow reason\"}"
        )
