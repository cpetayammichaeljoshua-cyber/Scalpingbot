...
# v56.0: claude-fable-5 (11th) + claude-mythos-5 (12th) added — BOTH 404, replaced v82.0
# v82.0: GODMODE_STRUCTURAL_VORTEX + GODMODE_MACRO_NEXUS (gpt-oss-120b/20b) → 12 combos
# v85.0: GODMODE_FABLE5(qwen3-235b) + GODMODE_MYTHOS5(llama-3.3-70b) → 14 combos
# v86.0: 3 storm-disabled combos replaced:
...
GODMODE_COMBOS = [
    ...
    {
        # REPLACED (v86.0 2026-06-12): z-ai/glm-4.5-air:free → consistently "unavailable" errors
        # (7 consecutive unavailable in live logs — not rate_limit, model simply offline/restricted).
        # REPLACED with: openai/gpt-oss-20b:free — confirmed fastest/most reliable free-tier model.
        # Contrarian personality preserved via system prompt — distinct from GODMODE_KIMI_RESEARCH (bull/bear synthesis).
        "id": "GODMODE_GPT20B_CONTRARIAN",
        "model": "openai/gpt-oss-20b:free",
        "system": (
            "You are a contrarian market analyst specialising in reversals and extremes. "
            "Identify overbought/oversold conditions, divergences, and exhaustion patterns. "
            "Step 1 — EXTREME CHECK: Is price at ATH/ATL proximity, F&G extremes (<20 or >80), or funding extreme? "
            "Step 2 — DIVERGENCE: Does volume confirm the move, or is it diverging (price up + vol down = trap)? "
            "Step 3 — REVERSAL PROBABILITY: Assess RSI, Bollinger extremes, VPIN. Is the crowd wrong? "
            "Contrarian conviction requires ≥2 of 3 factors aligned. No hedging. Decisive JSON signals only. "
            "JSON format: {\"vote\": \"BUY|SELL|NEUTRAL\", \"confidence\": 50-95, \"narrative\": \"reason\"}"
        ),
        "emoji": "🟠",
    },
    ...
    # REMOVED (v86.0 2026-06-12): nvidia/nemotron-3-super-120b-a12b:free → consistent rate_limit storm=10
    # (10 consecutive rate_limit errors → 240s storm-disable every cycle, disrupting GODMODE pool).
    # REPLACED with: openai/gpt-oss-120b:free — same 120B scale, confirmed stable free tier.
    # System prompt: macro regime framework distinct from GODMODE_FINROBOT_CHAIN (analyst chain) + GODMODE_STRUCTURAL_VORTEX (structure).
    {
        "id": "GODMODE_GPT120B_MACRO",
        "model": "openai/gpt-oss-120b:free",
        "system": (
            "You are a macro quantitative trading strategist. Evaluate institutional order flow, "
            "macro regime, and structural momentum alignment using a 3-layer framework: "
            "[1-REGIME] Is the current macro regime EXPANSION (positive funding, rising OI, BTC dominance trending) "
            "or CONTRACTION (negative funding, liquidation cascades, risk-off rotation)? "
            "[2-INSTITUTIONAL FLOW] Net order flow direction: aggressive buyers vs sellers at the tape. "
            "GEX positive (gamma dealers buy dips) vs negative (dealers sell rallies) — critical for direction bias. "
            "[3-MOMENTUM ALIGNMENT] Are short-term (1h) and medium-term (4h) momentum aligned? "
            "Alignment + institutional flow agreement → high-conviction directional. Divergence → NEUTRAL. "
    ...
    # gpt-oss-120b confirmed live winner score=93.5/100, latency=6190ms 2026-05-31.
    {
        ...
        "system": (
            ...
            "Check VPIN level: <0.30=clean retail flow (favorable), >0.60=informed toxic flow (adverse selection). "
            "HMM regime: TRENDING=continuation bias, RANGING=mean-reversion bias, VOLATILE=reduce size. "
            "Regime persistence: if same HMM state for 3+ consecutive cycles → structural trend (not noise). Vote: BUY/SELL/NEUTRAL. "
            "[2-TECHNICAL] EMA stack, RSI, MACD, Bollinger — trend direction and strength? Vote: BUY/SELL/NEUTRAL. "
            "[3-RISK] Max drawdown exposure, volume depth, stop-loss invalidation — is risk:reward ≥2.35? "
            "VPIN >0.65 = adverse selection → require R:R ≥3.0 (elevated threshold for toxic flow). "
            "MaxDD >35% portfolio → require R:R ≥3.5 (survival mode elevated threshold). "
            "VPIN <0.30 + R:R ≥2.65 = green-light favorable conditions. Vote: ACCEPT/REJECT. "
            "Spread health: wide spread (>0.75 pct) = adverse entry cost. Volume ratio >4.5 or <0.3 = liquidity crisis. "
            "[4-MOMENTUM] Rate-of-change, volume surge, breakout velocity — momentum confirmed? "
            "Quality surge: volume_ratio 1.5-2.5 + directional close = clean breakout momentum (high conviction). "
            "Exhaustion surge: volume_ratio >3.0 + price not advancing = distribution/reversal risk → NEUTRAL. "
            "OFI velocity: OFI z-score increasing trend = momentum building. Decreasing = momentum fading. Vote: BUY/SELL/NEUTRAL. "
    ...
    # v21.2 NEW: OpenBB-inspired macro synthesis — originally google/gemma-4-31b-it:free
    # REMOVED (v86.0 2026-06-12): google/gemma-4-31b-it:free → consistent rate_limit storm=10
    # (10 consecutive rate_limit errors → 240s storm-disable every cycle, confirmed in v86.0 live logs).
    ...
    # GODMODE_LLAMA_QUANT (trend/momentum) — macro 6-layer OpenBB framework preserved for ensemble diversity.
    {
        ...
        "system": (
            ...
            "[5-VPIN MICROSTRUCTURE] High VPIN (>0.65) = toxic order flow — informed traders dominating, adverse selection risk. "
            "Low VPIN (<0.35) = uninformed retail flow — lower adverse selection, safer directional entry. "
            "MaxDD context: portfolio drawdown >40% = risk-elevated environment → size down 25%, cap confidence at 75. "
...
class G0DM0D3Engine:
    ...
    _GENERIC_ERR_THRESHOLD   = 8      # v52.0: 8→12; v82.0: 12→5; v86.0: 5→8 — live logs confirmed
                                     # qwen3-72b:free and qwen3-235b:free were perm_disabled by only 5
                                     # generic errors during a storm cycle — losing the two backbone
                                     # free-tier MoE models gutted the CONSORTIUM to 3/8 healthy.
                                     # 8 errors: still fast-disable for truly broken routes (404 slugs)
                                     # while surviving transient infrastructure bursts. Historical:
                                     # v52.0=8→12(too permissive, storms drained), v82.0=12→5(too tight)
    _GENERIC_ERR_DISABLE_S   = 3600.0 # v86.0: 7200→3600 (1h). 2h perm-disable kept backbone models
                                     # (qwen3-72b, qwen3-235b) out for entire trading sessions after a
                                     # single storm. 1h matches the OpenRouter free-tier refresh window.
    ...
    _CONSORTIUM_TIMEOUT      = 22.0   # v100.0: 16.0→22.0 — gpt-oss models respond at 12-16s; dynamic floor raised 6→10s
    ...
    async def _run_consortium_mode(
        ...
        _dyn_timeout    = max(10.0, self._CONSORTIUM_TIMEOUT * _healthy_frac)  # v100.0: floor 6→10s
    ...
    async def analyze(
        ...
        system_prompt = (
            ...
            "(a) hour_utc is 1-7 (Asian session low-liquidity window — historical WR<24%, adverse selection risk), "
            "(b) volume_ratio > 3.0 (extreme overextension — vol surge fades >80% of the time), "
            "(c) rsi > 78 OR rsi < 22 (extreme RSI — mean-reversion probability >65% in next 4h), "
            "(d) |funding_rate| > 0.04% (extreme funding — imminent squeeze/cascade likely, not a clean directional), "
            "(e) portfolio MaxDD > 46% (deep drawdown crisis — survival mode, preserve capital, no new directional risk), "
            "(f) OFI z-score absolute value > 2.0 AND against signal direction (institutional opposition detected), "
            "(g) rolling WinRate < 20% over last 20 trades (systematic deterioration — halt until recovery), "
            "(h) EV of last 10 signals < -0.30R average (chronic negative EV = system is bleeding, halt new positions), "
            "(i) VPIN > 0.60 AND spread > 0.08% (toxic flow + wide spread = informed-trader ambush condition). "
            "Apply this 4-step EV framework ONLY if no hard NEUTRAL override fires: "
            "[1-REGIME] Is the macro+funding+OI regime EXPANSION (positive EV) or CONTRACTION (negative EV)? "
            "Funding >+0.07% = crowded longs = negative EV for longs. Funding <-0.07% = crowded shorts = neg EV shorts. "
            "OI rising + price moving = conviction. OI rising + price flat = manipulation/trap. "
            "HMM state BULL/BEAR/NEUTRAL weights regime score; BULL regime = prefer longs, BEAR = prefer shorts. "
            "Sharpe ratio context: SR>1.5 = healthy risk-adj environment; SR<0 = adverse; SR<-2.0 = crisis, strong caution. "
            "[2-FLOW] Does order flow (OFI direction, volume_ratio, depth imbalance) CONFIRM price direction? "
            "Volume_ratio 1.2-2.0 + directional OFI = institutional conviction (>2.0 = overextension/exhaustion risk). "
            "OFI z-score >1.5 aligned with direction = strong institutional flow confirmation (+8pp confidence). "
            "VPIN: >0.60 = toxic order flow (informed traders active, adverse selection risk) → reduce confidence 18pp. "
            "VPIN: 0.50-0.60 = elevated risk → reduce confidence 8pp. "
            "VPIN: <0.35 = uninformed retail flow → favorable adverse selection environment, mild +5pp confidence boost. "
            "Candle quality: 3+ consecutive closes confirming direction = momentum confirmation (+5pp). "
            "Volume exhaustion: volume_ratio >2.5 AND price stalling = reversal risk → NEUTRAL regardless of direction. "
            "Spread width: spread >0.08% of price = illiquid conditions → reduce confidence 12pp. Vote: BUY/SELL/NEUTRAL. "
            "[3-EV-SYNTHESIS] E[V] = P_win × 2.65 − (1 − P_win). At current WR≈29%: E[V]≈+0.06R — razor-thin edge. "
            "REQUIRE P_win≥38% minimum for a positive-EV signal (crisis calibration — WR<30% means every trade must be premium). "
            "Below 38% win probability OR regime vs flow disagreement → NEUTRAL. Default to NEUTRAL when uncertain. "
            "EMERGENCY DRAWDOWN OVERRIDE: MaxDD>40%→downgrade confidence 25pp. MaxDD>46%→force NEUTRAL (survival mode). "
            "WR CRISIS CAP: if WinRate < 29% → hard cap confidence at 63 regardless of other factors. "
            "Confidence: 55-65%=marginal edge(only if 3/3 signals align), 65-80%=confirmed edge, 80-90%=institutional conviction. "
            "NEVER output confidence above 70 unless BOTH regime AND flow AND OFI all confirm the same direction. "
            "[4-CONVICTION] Triple-check: regime + flow + OFI all aligned same direction = HIGH-CONVICTION signal. "
            "OFI z-score >2.0 aligned = institutional conviction; OFI z-score <-2.0 against = institutional rejection. "
            "Any two of three misaligned = reduce confidence by 25pp (and cap at 60). All three misaligned = NEUTRAL. "
            "Momentum persistence: 3+ consecutive aligned candles upgrades confidence by 6pp (cap 88, not 95). "
            "IRONS quality floor: if signal quality score < 68 (crisis floor active) → cap confidence at 70. "
            ...
            "FLOAM SIGNAL ORTHOGONALITY (PCA): Per Grinold's Fundamental Law IR = IC × √BR. "
            "When OFI, quality_score, and VPIN signals are highly cross-correlated (avg|corr|>0.70), they represent "
            "OVERLAPPING alpha — NOT independent breadth. Non-orthogonal signals inflate apparent conviction but degrade IR. "
            "If signals are collinear AND same-direction → reduce confidence 15pp (single-factor bubble masquerade). "
            "Truly orthogonal signals (avg|corr|<0.30) = genuinely independent breadth → FLOAM IR premium → +6pp conviction. "
            "IC TRACKING: Information Coefficient proxy = (rolling_WR - 0.50) × 2. "
