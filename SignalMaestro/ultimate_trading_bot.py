...
Combines all features: Signal generation, ML analysis, Telegram integration, Unity forwarding
...
try:
    from SignalMaestro.api_resilience_layer import (
        APIResilienceManager, TelegramAPIWrapper, BinanceAPIWrapper, UnityAPIWrapper,
...
class UltimateTradingBot:
    ...
    def __init__(self):
        ...
        self.unity_integration_config = {'enabled': True, 'signal_format': 'unity_compatible'}
            ...
            try:
                ...
                if API_RESILIENCE_AVAILABLE:
                    api_config = {
                        ...
                        'ccxt_webhook_url': os.getenv('CCXT_EXECUTOR_URL', '')
                    }
    ...
    async def load_external_config(self, config_path: Path) -> bool:
        ...
        try:
            ...
            if 'unity_integration' in self.external_config:
                self.unity_integration_config = self.external_config['unity_integration']
    ...
    def format_ml_signal_message(self, signal: Dict[str, Any]) -> str:
        ...
        # Simple Unity format
        unified_signal = self._format_unified_signal(signal)
...
🎯 Multi-Indicator Confluence | 🧠 ML Enhanced | 🛡️ Auto Risk Management"""
    ...
    def _format_unified_signal(self, signal: Dict[str, Any]) -> str:
        """Format signal in Unity-compatible format with improved parsing"""
        try:
            ...
            # Unity-compatible format with clear parsing structure
...
- CloseAll: AfterTP3"""
            ...
            self.logger.error(f"Error formatting Unity signal: {e}")
            optimal_leverage = signal.get('optimal_leverage', 35)
            # Fallback format that's still Unity-compatible
...
Exchange: BinanceFutures"""
    ...
    async def send_to_executor(self, signal: Dict[str, Any]) -> bool:
        """Unity integration disabled - signals sent via Telegram only"""
        return True
...
• Unity-readable format for automation
