...
Dynamically processes trading signals with AI analysis for Unity Engine
"""
            ...
            if any(kw in _err_str for kw in _auth_keywords):
                if not _openai_auth_failed:
                    ...
                    logging.getLogger(__name__).info(
                        "ℹ️  OpenAI API key invalid (401) — rule-based fallback active. "
                        "Primary AI runs via OpenRouter. Set OPENAI_API_KEY to re-enable."
                    )
            else:
        ...
        try:
            ...
            if any(kw in _err_str for kw in _auth_keywords):
                if not _openai_auth_failed:
                    _openai_auth_failed = True
                    logging.getLogger(__name__).info(
                        "ℹ️  OpenAI key invalid — sentiment using rule-based fallback."
                    )
            return {"sentiment": "neutral", "score": 0.0, "fallback": True}
...
class AIEnhancedSignalProcessor:
    """Enhanced signal processor with OpenAI integration for Unity Engine"""
    ...
    async def process_and_enhance_signal(self, raw_signal: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Format signal for Unity Engine dispatch
            unified_signal = self._format_signal(enhanced_signal)
...
"""
    ...
    def _format_signal(self, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Create Unity-compatible signal dict
        unified_signal = {
...
"""
    ...
    async def push_signal_to_channel(self, signal_dict: Dict[str, Any]) -> bool:
        """Push enhanced signal to Telegram channel"""
        try:
            message = signal_dict.get('formatted_message', '')
            if not message:
                return False
            ...
            if success:
                ...
                self.logger.info(f"✅ Signal pushed to channel: {signal_dict.get('symbol', '')}")
                ...
                self.logger.error(f"❌ Failed to push signal for {signal_dict.get('symbol', '')}")
                return False
