...
def _run_proxy_backtest(bars: List[KlineBar],
                        sl_pct: float = 0.0065,
                        tp_pct: float = 0.0110,
                        min_conf: float = 0.25) -> SimulationResult:  # v18.5: 0.58→0.25
    """
...
class MiroFishSimulationEngine:
    ...
    def __init__(
        ...
        sl_pct:       float = 0.0065,
        tp_pct:       float = 0.0110,
        min_conf:     float = 0.25,  # v18.5: 0.58→0.25 — proxy agents rarely hit 0.58
    ):
    ...
    async def _sweep_all(self) -> None:
        ...
        nodata = sum(1 for r in done if isinstance(r, SimulationResult) and r.error == "no_data")
        err    = sum(1 for r in done if not isinstance(r, SimulationResult) or (r.error and r.error != "no_data"))
        elapsed = time.time() - t0
        _nd_tag = f", {nodata} no_data" if nodata else ""
        self._logger.info(
            f"[MiroFishSim] Sweep #{self._run_count}: {ok}/{len(self.symbols)} ok"
            f"{_nd_tag}, {err} errors — {elapsed:.1f}s"
        )
        if err > 0:
            _err_syms = [
                (r.symbol if isinstance(r, SimulationResult) else "?") + (f"({r.error[:40]})" if isinstance(r, SimulationResult) else f"(exc:{str(r)[:40]})")
                for r in done if not isinstance(r, SimulationResult) or (r.error and r.error != "no_data")
            ]
            # v18.35: INFO-level so error symbol names appear in production Railway console
            self._logger.info(f"[MiroFishSim] Error symbols: {_err_syms}")
    ...
    async def _fetch_klines(self, symbol: str) -> List[KlineBar]:
        """
        v18.2 FIX: Added content_type=None to r.json() — Binance FAPI occasionally
        returns Content-Type: application/json;charset=UTF-8 which aiohttp's strict
        MIME check rejects as non-JSON, causing a ClientResponseError that the outer
        except block catches and returns [] → "no_data" for every symbol in the sweep.
        Also added one automatic retry with exponential backoff for transient 5xx/429
        responses, and re-creates a closed session on the fly so the engine never
        stalls after a network partition. Fixes "0/50 ok, 50 errors" sweep log.
        """
        if not _HAS_AIOHTTP:
            return []
        if self._session is None or self._session.closed:
            try:
                ...
                self._logger.debug("[MiroFishSim] _fetch_klines: re-created closed session")
            except Exception:
                return []
            ...
            try:
                async with self._session.get(url, params=params) as r:
                    if r.status == 429 or r.status >= 500:
                        if _attempt == 0:
                            await asyncio.sleep(2.0 + _attempt * 3.0)
                            continue
                        return []
                    if r.status != 200:
                        self._logger.debug(
                            f"[MiroFishSim] klines {symbol}: HTTP {r.status}"
                        )
                        return []
