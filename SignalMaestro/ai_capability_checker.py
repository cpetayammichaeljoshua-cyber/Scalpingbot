...
class AICapabilityChecker:
    ...
    def __init__(self):
        ...
        self.component_requirements = {
            ...
            # fallback_score 0.85 (sklearn MLP ensemble is genuinely capable).
            "pytorch_transformers": {
                "packages":  ["torch", "transformers"],
                "env_vars":  [],
                "intelligence_score": 1.00,  # v6.0: TransformerEncoder SOVEREIGN verified → 1.00
                "fallback_score":     0.85,  # v6.0: sklearn MLP ensemble highly capable → 0.85 (was 0.75)
                "critical": False,
            },
            # ── scikit-learn ───────────────────────────────────────────────────
        ...
        for component, requirements in self.component_requirements.items():
            ...
            if result.available:
                ...
                if component == "pytorch_transformers":
                    # torch absent (unexpected now) — log at warning level
                    self.logger.warning(
                        f"pytorch_transformers: torch/transformers absent — "
                        f"using sklearn fallback (score={requirements['fallback_score']:.2f}). "
                        f"Install via: pip install torch --index-url https://download.pytorch.org/whl/cpu"
                    )
                else:
    ...
    def _log_capability_results(self, capability: SystemCapability):
        ...
        for component, result in capability.components.items():
            ...
            if component == "sklearn" and result.available:
                ...
                # Actual DEGRADED state: pytorch absent, sklearn fallback at 0.85.
                score_label = (
                    f"score: {result.intelligence_score:.2f} "
                    f"(DEGRADED · pytorch absent → sklearn fallback · rebuild to restore pytorch SOVEREIGN 1.00)"
                )
            else:
