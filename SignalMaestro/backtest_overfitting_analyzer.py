...
Scores are combined into a single `OverfitAssessment` with:
  ...
  • `quality_penalty`     — [-7.0, 0.0] bias applied to Gate 8.5  [v18.76: -5→-7 stronger overfit veto]
...
_BOOTSTRAP_REPS      = 1000  # v18.76: 500→1000 bootstrap resamples — tighter PBO confidence intervals (Bailey & Lopez de Prado 2014)
...
class OverfitAssessment:
    ...
    quality_penalty:    float   = 0.0   # [-7, 0] bias for Gate 8.5  [v18.76: -5→-7]
...
class BacktestOverfittingAnalyzer:
    ...
    Quality penalty scale:
      CLEAN   (WFR ≥ 0.55, PBO < 0.55, DSR > 0)   →  0.0  (no penalty)  [v18.76: WFR 0.50→0.55]
      SUSPECT (WFR 0.10-0.55 OR PBO 0.55-0.70)    → -3.0  (soft warning)
      OVERFIT (WFR < 0.10  OR PBO > 0.70 OR DSR<0) → -7.0  (strong veto)  [v18.76: -5→-7]
    """
    ...
    def assess(
        ...
        if is_overfit:
            ...
            result.quality_penalty = -7.0   # v18.76: -5.0→-7.0 stronger overfit veto (Bailey & Lopez de Prado)
        elif is_suspect:
