...
Handles HTTP endpoints for Unity, TradingView, and other signal sources
"""
...
class WebhookServer:
    ...
    def __init__(self, telegram_bot, binance_trader, unity_integration, database):
        self.config = Config()
    ...
    def _setup_routes(self):
        ...
        @self.app.route('/webhook/unity', methods=['POST'])
        def ccxt_webhook():
            """Unity webhook endpoint for receiving signals"""
            try:
                data = request.get_json()
                if not data:
                    return jsonify({'error': 'No JSON data provided'}), 400
                
                result = asyncio.run(self._process_unified_signal(data))
                return jsonify(result)
                
            except Exception as e:
                self.logger.error(f"Error processing Unity webhook: {e}")
                return jsonify({'error': str(e)}), 500
    ...
    async def _process_unified_signal(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Process Unity webhook signal"""
        try:
            # Unity signals come in a specific format
            ...
            self.logger.error(f"Error processing Unity signal: {e}")
            return {
                'success': False,
                'error': str(e)
            }
        ...
        try:
            ...
            if self.config.AUTO_TRADE_ENABLED:
                ...
                if trade_result['success']:
                    ...
                    # Forward to Unity
                    unity_result = await self.unity.forward_signal(signal, trade_result)
                ...
                # Just forward to Unity without executing
                unity_result = await self.unity.forward_signal(signal)
        ...
        try:
            ...
            return jsonify({
                ...
                'components': {
                    ...
                    'unity': 'configured' if self.config.UNITY_BOT_UUID else 'not_configured'
                },
                'stats': {
