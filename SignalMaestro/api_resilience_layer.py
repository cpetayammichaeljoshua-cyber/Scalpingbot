...
class GracefulDegradation:
    ...
    def _get_service_default(self, service: str, api_call: APICall) -> Any:
        ...
        defaults = {
            ...
            "unity": {"success": False, "message": "Unity service unavailable"}
        }
...
class UnityAPIWrapper:
    """Resilient wrapper for Unity API calls"""
    ...
    async def _make_resilient_call(self, api_call: APICall) -> Dict[str, Any]:
        """Make resilient API call to Unity"""
        async def make_call():
            async with aiohttp.ClientSession() as session:
                async with session.post(self.webhook_url, json=api_call.payload,
                    ...
                    # Unity might return text or JSON
...
class APIResilienceManager:
    ...
    def _init_api_wrappers(self):
        ...
        # Unity wrapper
        if self.config.get("ccxt_webhook_url"):
            self.unity = UnityAPIWrapper(
                self.config["ccxt_webhook_url"],
                self.error_handler
            )
        else:
            self.unity = None
    ...
    def get_service_health(self) -> Dict[str, Any]:
        ...
        for service in ["binance", "telegram", "unity"]:
