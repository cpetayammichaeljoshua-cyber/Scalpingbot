...
def build_features(trade: Dict) -> "np.ndarray":
    ...
    # ── v39 (v107.0): F206-F210 — multi-source sentiment-regime features ──────────────
    # F206: fmp_ofi_wrc_norm  — G8.5P3 FundMom-OFI-WRCrisis TripleResonance output ±1
    _v39_f206 = _safe_float(trade.get("fmp_ofi_wrc_norm", 0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v39_f206)))                                  # 206 fmp_ofi_wrc_norm
    # F207: rsc_wrev_efo_norm — G8.5Q3 RegimeSent-WREV-EFO TripleCoherence output ±1
    _v39_f207 = _safe_float(trade.get("rsc_wrev_efo_norm", 0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v39_f207)))                                  # 207 rsc_wrev_efo_norm
    # F208: p3_q3_consensus — combined P3+Q3 consensus score [-1,+1]
    _v39_f208 = _safe_float(trade.get("p3_q3_consensus", 0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v39_f208)))                                  # 208 p3_q3_consensus
    # F209: regime_crisis_meta — composite regime pressure (WRC+EFO+RSC)/3 [-1,+1]
    _v39_f209 = _safe_float(trade.get("regime_crisis_meta", 0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v39_f209)))                                  # 209 regime_crisis_meta
    ...
    # ── v41 (v109.0): F216-F220 — T3/U3 regime-flow-spread-sync health features ──────────────
    # F216: t3_regime_flow_norm — G8.5T3 RegimeFlow-OFI-WR TripleAlignment output ±1
    _v41_f216 = _safe_float(trade.get("t3_regime_flow_norm", 0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v41_f216)))                                  # 216 t3_regime_flow_norm
    # F217: u3_kalman_spread_norm — G8.5U3 Kalman-HMM-Spread TripleSync output ±1
    _v41_f217 = _safe_float(trade.get("u3_kalman_spread_norm", 0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v41_f217)))                                  # 217 u3_kalman_spread_norm
    # F218: t3_u3_sync — combined T3+U3 consensus score [-1,+1]
    _v41_f218 = _safe_float(trade.get("t3_u3_sync", 0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v41_f218)))                                  # 218 t3_u3_sync
    # F219: regime_ofi_persistence — composite (TrendMom+RegimeSent+FundMomPersist)/3 [-1,+1]
    _v41_f219 = _safe_float(trade.get("regime_ofi_persistence", 0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v41_f219)))                                  # 219 regime_ofi_persistence
    ...
    # ── v48 (v117.0): F251-F255 — TimesFM-ForecastConfluence + RegimeSync features ──
    # F251: timesfm_dir_norm     — G8.5D4 patch-forecast direction {-1,0,+1}
    # F252: timesfm_conf_norm    — patch-consensus confidence [0,1]
    # F253: timesfm_slope_avg    — normalized 8-bar LinReg slope / ATR, clipped [-1,+1]
    # F254: timesfm_confluence   — G8.5D4 gate output {-1,0,+1}
    # F255: timesfm_regime_sync  — G8.5E4 gate output {-1,0,+1}
    _v48_f251 = _safe_float(trade.get("timesfm_dir_norm",         0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v48_f251)))                                      # 251 timesfm_dir_norm
    _v48_f252 = _safe_float(trade.get("timesfm_conf_norm",        0.0), 0.0)
    f.append(max(0.0,  min(1.0, _v48_f252)))                                      # 252 timesfm_conf_norm
    _v48_f253 = _safe_float(trade.get("timesfm_slope_avg",        0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v48_f253)))                                      # 253 timesfm_slope_avg
    _v48_f254 = _safe_float(trade.get("timesfm_confluence_gate",  0.0), 0.0)
    ...
    # ── v52 (v121.0): F271-F275 — EVVelocity + WRAccel + MaxDD-EV + KellyRegime + LossStreak ──
    # F271: ev_velocity_dir  — G8.5O4 EV-velocity direction {-1,0,+1}
    # F272: wr_accel_dir     — G8.5P4 WR acceleration direction {-1,0,+1}
    # F273: maxdd_ev_cmpd    — G8.5Q4 MaxDD-EV compound {-1,0,+1}
    # F274: kelly_regime_n   — normalized Kelly fraction (fraction×500−1, clipped [-1,+1])
    # F275: loss_streak_n    — consecutive losses / 10, negated clipped [-1,0]
    _v52_f271 = _safe_float(trade.get("ev_velocity_dir",  0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v52_f271)))                                      # 271 ev_velocity_dir
    _v52_f272 = _safe_float(trade.get("wr_accel_dir",     0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v52_f272)))                                      # 272 wr_accel_dir
    _v52_f273 = _safe_float(trade.get("maxdd_ev_cmpd",    0.0), 0.0)
    f.append(max(-1.0, min(1.0, _v52_f273)))                                      # 273 maxdd_ev_cmpd
    _v52_f274 = _safe_float(trade.get("kelly_regime_n",   0.0), 0.0)
    ...
    # ── v63 (v137.0): F326-F330 — OSW+FSL Triple-Momentum+Safety Features ────
    # F326: m5_osw_gate      — G8.5M5 OFI-Sharpe-WR Triple-Momentum gate {0.0/0.1/0.3/0.5/0.75/1.0}
    # F327: m5_ofi_momentum  — OFI ring mean normalised [-1,+1] (raw /3.0 clamped)
    # F328: m5_sharpe_vel    — Sharpe velocity from signal_data [-1,+1]
    # F329: n5_fsl_gate      — G8.5N5 Funding-Spread-Liquidity gate {0.0/0.1/0.3/0.5/0.75/1.0}
    # F330: n5_funding_stress — |funding_rate_trend|/0.0005 clamped [0,1] (0=neutral, 1=extreme)
    _v63_f326 = _safe_float(trade.get("m5_osw_gate",       0.5), 0.5)
    f.append(max(0.0,  min(1.0,  _v63_f326)))                                     # 326 m5_osw_gate
    _v63_f327 = _safe_float(trade.get("m5_ofi_momentum",   0.0), 0.0)
    f.append(max(-1.0, min(1.0,  _v63_f327)))                                     # 327 m5_ofi_momentum
    _v63_f328 = _safe_float(trade.get("m5_sharpe_vel",     0.0), 0.0)
    f.append(max(-1.0, min(1.0,  _v63_f328)))                                     # 328 m5_sharpe_vel
    _v63_f329 = _safe_float(trade.get("n5_fsl_gate",       0.5), 0.5)
    f.append(max(0.0,  min(1.0,  _v63_f329)))                                     # 329 n5_fsl_gate
    _v63_f330 = _safe_float(trade.get("n5_funding_stress", 0.0), 0.0)
    f.append(max(0.0,  min(1.0,  _v63_f330)))                                     # 330 n5_funding_stress

    # ── v64 (v138.0): F331-F335 — RSQ+EWV Triple-Coherence+Composite Features ─
    # F331: o5_rsq_gate     — G8.5O5 Regime-Signal-Quality gate {0.0/0.1/0.3/0.5/0.75/1.0}
    # F332: o5_qual_slope   — quality_score_ring linear slope normalised [-1,+1]
    # F333: o5_ofi_persist  — OFI persistence gate sentinel normalised [0=opposed, 0.5=neutral, 1=aligned]
    # F334: p5_ewv_gate     — G8.5P5 EV-WinRate-IRONS gate {0.0/0.1/0.3/0.5/0.75/1.0}
    # F335: p5_ev_trend     — booster ev_ring recent-5 vs prior-5 trend [-1,+1] (scaled /0.002)
    _v64_f331 = _safe_float(trade.get("o5_rsq_gate",   0.5), 0.5)
    f.append(max(0.0,  min(1.0,  _v64_f331)))                                     # 331 o5_rsq_gate
    _v64_f332 = _safe_float(trade.get("o5_qual_slope", 0.0), 0.0)
    f.append(max(-1.0, min(1.0,  _v64_f332)))                                     # 332 o5_qual_slope
    _v64_f333 = _safe_float(trade.get("o5_ofi_persist",0.5), 0.5)
    f.append(max(0.0,  min(1.0,  _v64_f333)))                                     # 333 o5_ofi_persist
    _v64_f334 = _safe_float(trade.get("p5_ewv_gate",   0.5), 0.5)
    ...
    # GDSA (161st gate): Direction-Selective Alpha — SHORT-side win-rate alpha detection
    # GEVL (162nd gate): EV Loss Velocity — EV-ring linear-regression slope monitoring
    # Technique 5 (Loop Engineering) + Technique 6 (Workflow Isolation).
    # Zero-padded backward-compat for trades stored before v173.0.
    # F406: bc_gdsa_gate      — GDSA state {-1.5→0.1, -1.0→0.3, 0→0.5, +1.0→0.7, +2.0→1.0} [0,1]
    # F407: short_alpha_score — SHORT-side raw WR alpha fraction [0,1]; 0.5 = neutral (no edge)
    # F408: bd_gevl_gate      — GEVL state {-2.0→0.0, -1.5→0.2, 0→0.5, +1.0→1.0} [0,1]
    # F409: ev_velocity_norm  — EV-ring slope normalised; 0.5 = flat, >0.5 = improving, <0.5 = worsening
    # F410: dir_ev_corr       — direction × EV-velocity cross-signal [0,1]; 0.5 = no correlation
    _v79_f406 = _safe_float(trade.get("bc_gdsa_gate",      0.5), 0.5)
    f.append(max(0.0, min(1.0,  _v79_f406)))                                      # 406 bc_gdsa_gate
    _v79_f407 = _safe_float(trade.get("short_alpha_score", 0.5), 0.5)
    f.append(max(0.0, min(1.0,  _v79_f407)))                                      # 407 short_alpha_score
    _v79_f408 = _safe_float(trade.get("bd_gevl_gate",      0.5), 0.5)
    f.append(max(0.0, min(1.0,  _v79_f408)))                                      # 408 bd_gevl_gate
    _v79_f409 = _safe_float(trade.get("ev_velocity_norm",  0.5), 0.5)
    f.append(max(0.0, min(1.0,  _v79_f409)))                                      # 409 ev_velocity_norm
    _v79_f410 = _safe_float(trade.get("dir_ev_corr",       0.5), 0.5)
    f.append(max(0.0, min(1.0,  _v79_f410)))                                      # 410 dir_ev_corr

    # ── v80 (v174.0): F411-F415 — GRDC+GXWI Role-Workflow Coherence Features ─
    # GRDC (163rd gate): Role-Defined Drawdown Coherence — risk-architect persona
    #   cross-validates WR trajectory (_win_ring) vs quality_score_ring slope.
    # GXWI (164th gate): XML-Workflow Isolation Divergence — Maker (live score)
    #   vs Checker (isolated historical baseline) divergence detection.
    # Technique 1 (Role Definition) + Technique 2 (XML Delimiters) +
    # Technique 3 (Extended Thinking) + Technique 6 (Workflow Isolation).
    # Zero-padded backward-compat for trades stored before v174.0.
    # F411: be_grdc_gate          — GRDC state {-2.0→0.0, -1.0→0.3, 0→0.5, +1.0→1.0} [0,1]
    # F412: wr_qs_coherence       — WR-trajectory × quality-slope coherence [0,1]; 0.5 = neutral
    # F413: bf_gxwi_gate          — GXWI state {-1.5→0.1, -1.0→0.3, 0→0.5, +0.5→0.8} [0,1]
    # F414: maker_checker_div     — Maker/Checker score divergence normalised [0,1]; 0.5 = aligned
    # F415: role_workflow_composite — combined GRDC×GXWI coherence composite [0,1]; 0.5 = neutral
    _v80_f411 = _safe_float(trade.get("be_grdc_gate",           0.5), 0.5)
    f.append(max(0.0, min(1.0,  _v80_f411)))                                      # 411 be_grdc_gate
    _v80_f412 = _safe_float(trade.get("wr_qs_coherence",        0.5), 0.5)
    f.append(max(0.0, min(1.0,  _v80_f412)))                                      # 412 wr_qs_coherence
    _v80_f413 = _safe_float(trade.get("bf_gxwi_gate",           0.5), 0.5)
    f.append(max(0.0, min(1.0,  _v80_f413)))                                      # 413 bf_gxwi_gate
    _v80_f414 = _safe_float(trade.get("maker_checker_div",      0.5), 0.5)
