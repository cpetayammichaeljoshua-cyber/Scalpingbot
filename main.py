...
- Production Signal Bot (Unity Engine native Telegram signals)
...
# Strip invisible Unicode characters (e.g. U+200E LEFT-TO-RIGHT MARK,
# U+200F, U+200B-U+200D, U+FEFF) that can appear in env-var KEY names when
# secrets are pasted from certain editors/browsers. Without this, callers
# look up "BINANCE_API_KEY" but the variable is actually stored as
# "\u200eBINANCE_API_KEY" and lookup returns None.
# Run BEFORE any other modules read os.environ.
# ─────────────────────────────────────────────────────────────────────────────
_INVISIBLE = ("\u200b", "\u200c", "\u200d", "\u200e", "\u200f", "\ufeff")
for _k in list(os.environ.keys()):
    _clean = _k
    for _c in _INVISIBLE:
        _clean = _clean.replace(_c, "")
    _clean = _clean.strip()
    if _clean and _clean != _k and _clean not in os.environ:
        os.environ[_clean] = os.environ[_k]
...
║  - Unity Engine Native Telegram Signals                      ║
...
class AITradingOrchestrator:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Send comprehensive startup test message
            await self.telegram_bot.send_startup_test_message()
