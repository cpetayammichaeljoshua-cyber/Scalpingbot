...
async def main():
    ...
    # ── Startup Banner ──
    logger.info("=" * 90)
    logger.info("🐟 MIROFISH SWARM + G0DM0D3 AI TRADING BOT v7.0 — ALL USDM MARKETS — PRODUCTION")
    logger.info("=" * 90)
    ...
    logger.info("🤖 AI:         G0DM0D3 v5.0 Framework [PRIMARY] — z4ptacticsbot integrated")
    logger.info("               github.com/elder-plinius/G0DM0D3")
    logger.info("               github.com/cpetayammichaeljoshua-cyber/z4ptacticsbot")
    logger.info("               • ULTRAPLINIAN: 26 free models, 5 tiers, full cascade")
    logger.info("                 fast(6)→standard(9)→smart(12)→power(19)→ultra(26)")
    logger.info("               • AutoTune: volatile/trending/ranging/breakout/news contexts")
    logger.info("               • STM: think_stripper+hedge_reducer+direct_mode+json_enforcer")
    logger.info("               • GODMODE CLASSIC: 5 distinct models (v5.0 lineup):")
    logger.info("                 🟣 Hermes 405B | 🔵 Llama 3.3 70B | 🟢 Qwen3 Next 80B")
    logger.info("                 🟡 QwQ-32B (reasoning) | 🟠 Gemma 3 27B")
    logger.info("               • EnsembleVote: majority-vote across all responses")
    logger.info("               • PerModelBucket: 7 calls/min per model (429 prevention)")
    logger.info("               • AutoReset: 90s cooldown guard (no infinite reset loop)")
    logger.info("               • ErrorTypeTrack: auth=24h, 429=X-RateLimit-Reset, 503=45s")
    logger.info("               • GenericErrGuard: 8 generic errors → 2h disable (Moonlight fix)")
    logger.info("               • max_retries=0: no 'Task exception was never retrieved'")
    logger.info("               • Global throttle: 80/min (raised from 50 — 26 models = 5× capacity)")
    logger.info("               • AI gate: 5/9 agents (was 7/9=never-triggered — fixed)")
    logger.info("               • G0DM0D3 timeout: 35s (was 20s — handles full cascade)")
    logger.info("               • Free Models (26): Hermes·Llama·Qwen3Next·QwQ32B·Qwen3-30B")
    logger.info("                 StepFun·Qwen3Coder·TrinityLarge·Qwen3-14B·Qwerky72B·Moonlight")
    logger.info("                 Dolphin·GLM4.5Air·Gemma3-27B·MAI-DS-R1·GLM-Z1·Qwen3-8B")
    logger.info("                 TrinityMini·LFM-Think·Qwen3-4B·Gemma2-9B·Devstral·LFM-Instruct")
    logger.info("                 Llama3.2-3B·UI-TARS-72B·Auto-Router")
    logger.info("⏱️  Timeframe:  15M (primary swing/scalp timeframe)")
    logger.info("📢 Channel:    @ichimokutradingsignal | InsiderTactics")
    logger.info("📋 Format:     Unity Engine native signal format")
    ...
    logger.info("   🐟 TrendAgent        — EMA 9/21 crossover + EMA200 + graph TrendState node    (18% w)")
    logger.info("   ⚡ MomentumAgent      — RSI + MACD + IndicatorState node → graph              (16% w)")
    logger.info("   📊 VolumeAgent       — OBV + vol surge + Catalyst node on 2x spike            (15% w)")
    logger.info("   🌊 VolatilityAgent    — BB + ATR + PriceLevel nodes (BB_Upper/Lower)          (12% w)")
    logger.info("   🕯️  OrderFlowAgent    — Candle patterns + Pattern graph nodes                 (12% w)")
    logger.info("   😱 SentimentAgent     — Fear/greed proxy + vol contraction regime             ( 4% w)")
    ...
    logger.info(f"   • Min R:R Ratio:   1.55:1 (any signal below this is rejected, was 1.50)")
