...
# ── v8.0 Railway-safe logging ─────────────────────────────────────────────────
# Use /tmp for the log file — /app may be read-only under non-root Railway
# containers. Falls back gracefully to stdout-only if even /tmp is unavailable.
_utbot_handlers = [logging.StreamHandler(sys.stdout)]
try:
    _utbot_handlers.append(logging.FileHandler('/tmp/ut_bot_signals.log'))
except (OSError, PermissionError):
    pass  # stdout-only — never crash on log setup

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=_utbot_handlers,
)
